...
tcga_sample_raw <- read.xlsx(
  xlsxFile <- "tcga_sample.xlsx",
  sheet <- 1,
  colNames <- TRUE,
  rowNames <- FALSE
)
tcga_sample <- tcga_sample_raw
colnames(tcga_sample)[4] <- "wsi_name"

dir<-'result_tcga'
all_files <- list.files(path <- dir, pattern <- '*.csv$',recursive <- TRUE)
all_files_dir <- paste0('result_tcga\\',all_files)
...
for (i in 1 : length(all_files_dir)) {
 current_data <- read.csv(all_files_dir[i], stringsAsFactors <- FALSE)
  ...
  tcga_sample[[new_col_name_CC1]] <- merged_data$"X0..."
  tcga_sample[[new_col_name_CC2]] <- merged_data$"X1..."
  tcga_sample[[new_col_name_CC3]] <- merged_data$"X2..."
}
...
colnames(tcga_sample)[4] <- "slide_wsi_name.svs"

select_factor <- c("BRAF_V600E", "RAS", "TERT_promoter")

for (i in select_factor) {
  tcga_sample[,i] <- as.factor(tcga_sample[,i])
}

save(tcga_sample, file <- "tcga_sample_df.Rdata")